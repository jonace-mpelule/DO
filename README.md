...
`do-ts` is the npm package name; `do` is the command it installs. Bun is
required to run it.

```bash
npm install --global do-ts
```

You can also install it globally with Bun:

```bash
bun install --global do-ts
```
...
The package is published as `do-ts`, but its `bin` mapping installs the command
...
Package names are first-come, first-served. Verify `do-ts` again immediately
